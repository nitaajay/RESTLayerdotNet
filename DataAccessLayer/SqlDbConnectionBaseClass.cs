...
namespace DataAccessLayer
{
    public sealed class SqlDbConnectionBaseClass : IDisposable
    {
        ...
        public DataTable ExecuteForSelect(string sSQL)
        {
            using (_dtTable = new DataTable())
            {
                using (_connection = OpenConnection())
        ...
        public DataTable ExecuteProcedureForSelect(string spName, SqlParameter[] param, string optional)
        {
            ...
            {
                Console.Write("In SQL Connection: ==============================================");
        ...
        //required for clsAutoDiscovery(added by aj)
        public int ExecuteNonQuery(string spName, SqlParameter[] param, string optional)
        {
            using (_dtTable = new DataTable())
            {
                using (_connection = OpenConnection())
                {
                    using (var sqlCommand = new SqlCommand(spName, _connection) { CommandType = CommandType.StoredProcedure})
                    {
                        foreach (var p in param)
                        {
                        ...
                        SqlParameter retval = sqlCommand.Parameters.Add("@return_value", SqlDbType.VarChar);
                        retval.Direction = ParameterDirection.ReturnValue;
                        sqlCommand.ExecuteNonQuery();
                        int retVal = (int) sqlCommand.Parameters["@return_value"].Value;
                        return retVal;
                    }
                }
            }
        ...
        //required for clsAutoDiscovery(added by Ajay)
